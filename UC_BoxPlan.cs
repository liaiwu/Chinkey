...
namespace SSAPP
{
    public partial class UC_BoxPlan : UCEditBase
    {
        ...
        RepositoryItemButtonEdit EditItem_Bxno;
        public UC_BoxPlan()
        {
            InitializeComponent();
            base.printBh = "08103";
        ...
        private void myInit()
        {
            ...
            //dataGrid.MainGridView.OptionsNavigation.UseOfficePageNavigation = true;
            dataGrid.UseEmbeddedNavigator= true;
            ...
            GridView gridview = dataGrid.MainView as GridView;
            ...
            this.toolTipController1.AutoPopDelay = 6000;//默认值是5000
            this.toolTipController1.InitialDelay = 300;//默认值是500
            this.toolTipController1.GetActiveObjectInfo += toolTipController1_GetActiveObjectInfo;
            this.dataGrid.ToolTipController = this.toolTipController1;

        }
        // 鼠标悬停提示
        void toolTipController1_GetActiveObjectInfo(object sender, ToolTipControllerGetActiveObjectInfoEventArgs e)
        {
            if (e.SelectedControl != this.dataGrid) return;
            ToolTipControlInfo info = null;
            try
            {
                GridView view = dataGrid.GetViewAt(e.ControlMousePosition) as GridView;
                if (view == null) return;
                GridHitInfo hi = view.CalcHitInfo(e.ControlMousePosition);
                if (hi.InRowCell)
                {
                    string content=GetCellHintText(view, hi.RowHandle, hi.Column);
                    if(content==string.Empty)return;
                    info = new ToolTipControlInfo(new CellToolTipInfo(hi.RowHandle, hi.Column, "cell"), content);
                    return;
                }
                //if (hi.Column != null)
                //{
        ...
        const string sql_tf_box = "select PRD_NAME,COLOR_NAME,SIZE_NAME,QTY from ZZ_TF_BOXDEF WHERE BX_NO='{0}'";
        private string GetCellHintText(GridView view, int rowHandle, DevExpress.XtraGrid.Columns.GridColumn column)
        {
            string bxno = view.GetRowCellDisplayText(rowHandle, "BX_NO").Trim();
            if (string.IsNullOrEmpty(bxno)) return string.Empty;
            if (bxno.Length == 0) return string.Empty;

            DataTable dt = myServer.GetDataTable(string.Format(sql_tf_box, bxno));
            strsb.Length = 0;
            string str1 = string.Empty;
            foreach (DataRow dr in dt.Rows)
            { 
                str1=dr["PRD_NAME"].ToString();
                str1=str1.PadLeft(10);
                strsb.Append(str1);
                str1 = dr["SIZE_NAME"].ToString();
                str1 = str1.PadLeft(10);
                strsb.Append(str1);
                str1 = dr["COLOR_NAME"].ToString();
                str1 = str1.PadLeft(10);
                strsb.Append(str1);

                str1 = dr["QTY"].ToString();
                str1 = str1.PadLeft(10);
                strsb.Append(str1);
        ...
        void dataGrid_OnRowDeleting(object sender, RowObjectEventArgs e)
        {
            dataGrid.DeleteRow(e.RowHandle);

        }
        
        DevGridColLookUpEdit devgridlookup;
        void gridview_CustomRowCellEditForEditing(object sender, CustomRowCellEditEventArgs e)
        {

            if (e.Column.FieldName == "BX_NO")
            {
                CWGridColumn column = e.Column as CWGridColumn;
                if (column == null) return;
                //if (EditItem_BX_NO == null)
                //{
                //    devgridlookup = DevColumnHelper.GetLookupItem(column.ColumnInfo);
                //    EditItem_BX_NO = devgridlookup;//DevColumnHelper.GetLookupItem(column.ColumnInfo);
                ...
                //string sql = "select BX_NO as 包装编号,BXNAME as 包装名称, CUS_OS_NO as 合约号, BAT_NO as  款式,BX_Type as 包装方式  from dbo.ZZ_MF_BOXDEF where CUS_OS_NO like '%"
                //    + this.txt_Cont_Id.Text + "%' and BAT_NO like '%" + this.txt_Style.Text + "%' ";
                //EditItem_BX_NO.DataSource = myServer.GetDataTable(sql);
                //e.RepositoryItem = EditItem_BX_NO;

                if (EditItem_Bxno == null)
                {
                    EditItem_Bxno = new RepositoryItemButtonEdit();
                    EditItem_Bxno.ButtonClick += EditItem_Bxno_ButtonClick;
                }
                e.RepositoryItem = EditItem_Bxno;
            }
        }
        Fm_SelectBoxDefList fmselectbox;
        void EditItem_Bxno_ButtonClick(object sender, DevExpress.XtraEditors.Controls.ButtonPressedEventArgs e)
        {
            if (null == fmselectbox || (null != fmselectbox && fmselectbox.IsDisposed))
            {
                fmselectbox = new Fm_SelectBoxDefList();
                fmselectbox.Cont_Id = this.txt_Cont_Id.Text;
                fmselectbox.Style_Id = this.txt_Style.Text;
                fmselectbox.LoadData();
                fmselectbox.OnSelect += fmselectbox_OnSelect;
                fmselectbox.Owner = this.FindForm();

            }
            fmselectbox.Show();
        }

        void fmselectbox_OnSelect(object sender, EventArgs e)
        {
            DataRow[] rows = fmselectbox.GetSelectRows();
            if(null==rows)return;
            string boxno = rows[0]["BX_NO"].ToString();
            dataGrid.SetRowValue("BX_NO", boxno);
            LoadBox(boxno, dataGrid.FocusedRowHandle);
            ...
            if (rows.Length > 1)
            {
                ...
                {
                    dataGrid.AddNewRow();
                    boxno = rows[r]["BX_NO"].ToString();
                    dataGrid.SetRowValue(DevExpress.XtraGrid.GridControl.NewItemRowHandle, "BX_NO", boxno);
                    LoadBox(boxno, DevExpress.XtraGrid.GridControl.NewItemRowHandle);
                }
            }
        }
        private void LoadBox(string boxno, int rowhandle)
        {
            MF_BOXDEF boxdef = MF_BOXDEF.GetById<MF_BOXDEF>(boxno);
            if (boxdef.BX_NO.Length > 0)
            {
                DataRow dr = dataGrid.GetDataRow(rowhandle);
        ...
        protected override object GetPrintData()
        {
            ...
            //rdt_tf.TableName = "表身";
            ReportDataTable rdt_tf2 = ReportDataTable.ConvertDataTable(this.curdtjc);
            rdt_tf2.TableName = "交叉表明细";

            ReportDataSet dataset = new ReportDataSet();
            dataset.Tables.Add(rdt_tf);
            dataset.Tables.Add(rdt_mf);
            dataset.Tables.Add(rdt_tf1);
            dataset.Tables.Add(rdt_tf2);
            //dataset.Tables.Add(rdt_tf);
  ...
  t.P2Name,t.P2QTY,t.P4Name,t.P4QTY,t.PColorName,t.PSizeName
  FROM ZZ_MF_BOXDEF m left join ZZ_TF_BOXDEF t on t.BX_NO=m.BX_NO
  WHERE m.BX_NO ='{1}' order by Color_ID,Size_Id";
        ...
        const string sql_all = @"select M.*,T.*,B.* from ZZ_MF_BOXBILL M left outer join ZZ_TF_BOXBILL T on T.BZ_NO=M.BZ_NO
LEFT OUTER JOIN ZZ_MF_BOXDEF B ON B.BX_NO=T.BX_NO 
WHERE M.BZ_NO='{0}'";
        /// <summary>
        /// 展开交叉表
        /// </summary>
        private void ExpandMXJC()
        {
            
            sbsql.Length = 0;
            string bzno = txt_BoxBNo.Text;
            string sql = string.Format(sql_all, bzno);

            DataTable mfjcdt = myServer.GetDataTable(sql);

            curdtjc = mfjcdt.Clone();

            string bxno;
            int qty;
            int nid;
            if (!curdtjc.Columns.Contains("Color_Name"))
            {
                curdtjc.Columns.Add("Color_Name", typeof(string));
            }
            foreach (DataRow dr in mfjcdt.Rows)
            {
                if (dr.RowState == DataRowState.Deleted)
                {
                    continue;
                ...
                qty = 0;
                nid = 0;
                qty=DataTableHelper.GetRowInt(dr, "BZ_QTY");
                nid = DataTableHelper.GetRowInt(dr, "NID");
                dr["NID"] = nid;

                sql = string.Format(sql_tf_boxdef, qty, bxno);
                DataTable dtmx = myServer.GetDataTable(sql);
                string col_sizename = string.Empty;
                string sizename = string.Empty;
                string colorname = string.Empty;
                bool isFind = false;
                foreach (DataRow mxdr in dtmx.Rows)
                {
                    sizename = mxdr["Size_Name"].ToString();
                    colorname = mxdr["Color_Name"].ToString();
                    col_sizename = "Size_" + sizename;
                    if (!curdtjc.Columns.Contains(col_sizename))
                    {
                        curdtjc.Columns.Add(col_sizename, typeof(decimal));
                    }
                    isFind = false;
                    DataRow findr = null;
                    ///开始查找颜色
                    foreach (DataRow jcdr in curdtjc.Rows)
                    {
                        if ((int)jcdr["NID"] == nid && (int)jcdr["BZ_QTY"] == qty && jcdr["BX_NO"].ToString().Equals(bxno, StringComparison.OrdinalIgnoreCase) && jcdr["Color_Name"].ToString() == colorname)
                        {
                            findr = jcdr;
                            isFind = true;
                            break;
                    ...
                    if (!isFind)
                    {
                        findr = curdtjc.NewRow();
                        DataTableHelper.OverwriteDataRow(dr, findr);
                        curdtjc.Rows.Add(findr);
                    }
                    findr[col_sizename] = mxdr["BX_QTY"];
                    findr["Color_Name"] = colorname;
                }

            }
            curdtjc.AcceptChanges();

        }
        const string sql_tf_boxdef_group2P = @"  SELECT count(t.bat_no) as count1,m.BX_NO,t.PRD_NAME,t.ORDER_NO,t.PColorName,sum(isnull(t.QTY,0)) as BX_QTY,sum(isnull(t.QTY,0))*{0} as QTY ,max(t.BARCODE) as BARCODE,
